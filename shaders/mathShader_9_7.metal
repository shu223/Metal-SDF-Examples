...
float sceneSDF_9_7(float3 p, float time){
    float t = 0.3 + 0.2 * sin(time);
    float d1 = octaSDF(p, 0.5);
    float d2 = sphereSDF(abs(p), float3(t), 0.1);
    return smin(d1, d2, 0.1);
}

[[ stitchable ]] half4 mathGraphicsShader_9_7(float2 position,
                                              half4 color,
                                              float4 boundingRect,
                                              float time)
{
    ...
    // レイを進めるループ
    for(int i = 0; i < 50; i ++ ) {
        if (sceneSDF_9_7(rPos, time) > 0.001) { // 球にぶつかる前
            // レイをさらに進める
            rPos += sceneSDF_9_7(rPos, time) * ray;
        } else { // レイが球にぶつかったところで接平面でのライティングを計算
            // 環境光の強さ
            float amb = 0.1;
            // 平行光による拡散光の強さの計算
            // 平行光と法線の内積を取る（法線と同じ角度で入社すると一番強い）
            float diff = 0.9 * max(dot(normalize(lDir), gradSDF(rPos, time, sceneSDF_9_7)), 0.0);
